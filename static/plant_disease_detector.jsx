...
const HINDI_LABELS = {
  "Anthracnose": "एन्थ्रेक्नोज रोग",
  "Bacterial_Canker": "बैक्टीरियल कैंकर रोग",
  "Cutting_Weevil": "कटिंग वीविल कीट",
  "Die_Back": "डाइ बैक रोग",
  "Gall_Midge": "गॉल मिज कीट",
  "Powdery_Mildew": "पाउडरी मिल्ड्यू रोग",
  "Sooty_Mould": "सूटी मोल्ड रोग",
...
// 2. MODEL PATHS
const MODEL_PATH = './mango_model.onnx';
const MODEL_DATA_PATH = './mango_model.onnx.data';
...
function PredictionResult({ result }) {
  ...
  if (result.disease === "Unknown") {
    return (
      <div className="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-300">
        <p className="text-xl font-bold text-yellow-700">⚠️ मान्य आम की पत्ती नहीं मिली</p>
        <p className="text-gray-600 mt-2">कृपया आम के पत्ते की स्पष्ट फोटो अपलोड करें।</p>
      </div>
    );
  }
  const diseaseName = HINDI_LABELS[result.disease] || result.disease;
  ...
  return (
    <div className="text-center">
      <h3 className="text-xl font-medium text-gray-700">रोग पहचान:</h3>
      <p className={`text-3xl font-bold ${colorClass}`}>{diseaseName}</p>

      <h3 className="text-xl font-medium text-gray-700 mt-4">विश्वसनीयता:</h3>
...
function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!dataResponse.ok)
          throw new Error(`Failed to fetch model.onnx.data: ${dataResponse.status}`);
  ...
  const postprocessOutput = (output) => {
    ...
    const isConfused = (sorted[0] - sorted[1]) < 0.2;

    if (maxProb < 0.90 || isConfused || CLASS_NAMES[maxIndex] === 'Other') {
      return {
        disease: "Unknown",
        confidence: maxProb,
      };
  ...
  const runPrediction = async () => {
    ...
    try {
      ...
      if (!result.disease) {
        setPredictError("Model output mismatch with CLASS_NAMES");
        return;
      }
    ...
    <div className="max-w-2xl mx-auto my-10 p-8 bg-white rounded-lg shadow-xl">
      ...
      <div className="space-y-4">
        <label htmlFor="file-upload" className="block text-sm font-medium text-gray-700">
          आम के पत्ते की फोटो अपलोड करें:
        </label>
        <input 
          id="file-upload" 
          type="file" 
        ...
        >
          {predictLoading ? 'Analyzing...' : 'रोग की पहचान करें'}
        </button>
      </div>
        ...
        {prediction && !prediction.disease.toLowerCase().includes('healthy') && (
          <button
            ...
            className="w-full flex justify-center py-3 px-4 border border-transparent
                       ...
                       disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
            {treatmentLoading ? 'उपचार योजना प्राप्त की जा रही है...' : '✨ उपचार योजना प्राप्त करें'}
          </button>
        )}
      </div>
